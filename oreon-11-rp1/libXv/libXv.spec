...
%global source0_file libXv-1.0.13.tar.xz
...
%(f=%{_sourcedir}/libXv-1.0.13.tar.xz; test -f "$f" || { echo "oreon: missing Source0 $f" >&2; exit 1; }; h=$(sha256sum "$f" | awk '{print $1}'); test "$h" = "7d34910958e1c1f8d193d828fea1b7da192297280a35437af0692f003ba03755" || { echo "oreon: Source0 SHA256 mismatch for libXv-1.0.13.tar.xz" >&2; exit 1; })
