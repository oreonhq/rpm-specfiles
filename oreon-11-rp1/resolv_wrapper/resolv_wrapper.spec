...
%global source0_file resolv_wrapper-1.1.8.tar.gz
...
%(f=%{_sourcedir}/resolv_wrapper-1.1.8.tar.gz; test -f "$f" || { echo "oreon: missing Source0 $f" >&2; exit 1; }; h=$(sha256sum "$f" | awk '{print $1}'); test "$h" = "fbc30f77da3e12ecd4ef66ccf5ab77e0b744930ccd89062404082f928a8ec2e0" || { echo "oreon: Source0 SHA256 mismatch for resolv_wrapper-1.1.8.tar.gz" >&2; exit 1; })
