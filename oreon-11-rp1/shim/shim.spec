...
%global vendor_cert_str %{expand:%%{!?vendor_cert_nickname:-c "Oreon Secure Boot Test Certificate"}%%{?vendor_cert_nickname:-c "%%{vendor_cert_nickname}"}}

%global grub_version 2.06-63
%global fwupd_version 1.5.8

%define __pesign_client_cert oreon-grub2-signer
...
%global shimveraa64 16.1-8
%global shimverarm 15.4-1.fc34
%global shimveria32 15.8-9
%global shimverx64 15.8-9
...
bootloader under Secure Boot. This build is signed for Oreon Linux	\
(Oreon signing / pesign).						\
%{nil}
...
install -m 0644 %{SOURCE0} $RPM_BUILD_ROOT%{_sysconfdir}/dnf/protected.d/
