...
test "%{source0101_hash}" = "none" || { f="%{SOURCE101}"; test -f "$f" || { echo "oreon: missing Source101 $f" >&2; exit 1; }; h=$(sha256sum "$f" | awk '{print $1}'); test "$h" = "%{source0101_hash}" || { echo "oreon: Source101 hash mismatch" >&2; exit 1; }; }
cp -da %{_sourcedir}/* .
tar xvf test.tar.gz
