...
for file in $files; do
  ...
  rpm2cpio $file | cpio -idmv ./usr/src/debug/${snvra}/build/{debug,optimized}/jit_stencils-${arch}-unknown-linux-gnu.h
  for build in debug optimized; do
    mv -v ./usr/src/debug/${snvra}/build/${build}/jit_stencils-${arch}-unknown-linux-gnu.h Python-${version}-${arch}-${build}-jit_stencils.h > /dev/stderr
