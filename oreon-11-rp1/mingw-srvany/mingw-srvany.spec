...
%(f=%{_sourcedir}/v1.1.tar.gz; test -f "$f" || { echo "oreon: missing Source0 $f" >&2; exit 1; }; h=$(sha256sum "$f" | awk '{print $1}'); test "$h" = "44e861476310ea1bab4dedf0a7736ba906e0037e7950909b9c48eadf72f7c170" || { echo "oreon: Source0 SHA256 mismatch for v1.1.tar.gz" >&2; exit 1; })
