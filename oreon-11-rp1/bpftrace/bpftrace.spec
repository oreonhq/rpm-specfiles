...
%global source0_file bpftrace-0.24.2.tar.gz
...
%(f=%{_sourcedir}/bpftrace-0.24.2.tar.gz; test -f "$f" || { echo "oreon: missing Source0 $f" >&2; exit 1; }; h=$(sha256sum "$f" | awk '{print $1}'); test "$h" = "e0514aa3e1a032b0b2de2cf3c281bfee9b9e80509498e70ed78786bbd64db373" || { echo "oreon: Source0 SHA256 mismatch for bpftrace-0.24.2.tar.gz" >&2; exit 1; })
