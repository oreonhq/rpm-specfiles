...
NSSDB=db
BACKENDS="softhsm opencryptoki libcacard"
...
function card_setup() {
    case $1 in
        ...
        "libcacard")
            ...
            rlRun "rm -f /usr/share/p11-kit/modules/opensc.module"
...
rlJournalStart
    rlPhaseStartSetup "General setup"
        rlAssertRpm $PACKAGE
        rlRun "yes | $DNF copr --hub fedora enable jjelen/vsmartcard"
        # libcacard is not available in normal repos since RHEL9 -- only in the above copr
        rlRun "$DNF install -y virt_cacard virtualsmartcard libcacard"
        DATA_SIZE=1024
    rlPhaseEnd
    ...
    for BACKEND in $BACKENDS; do
        ...
        # SHA1 is deprecated and removed since Fedora 41
            ...
            for SIGN_KEY in "0001" "0002" "0003"; do
                ...
                if [[ "$BACKEND" != "opencryptoki" ]]; then
                    rlPhaseStartTest "$BACKEND: $METHOD: Sign & Verify (KEY $SIGN_KEY)"
                        rlLogInfo "generate data: RSA-PKCS works only on small data"
                        rlRun "head -c 64 </dev/urandom > data"
                        if [[ ! -z $HASH ]]; then
                            rlLogInfo "generate data: hash- methods should work on data > 512 bytes"
                            rlRun "head -c $DATA_SIZE </dev/urandom > data"
                        fi
                ...
                METHOD="$METHOD-PSS"
                if [[ "$HASH" == "SHA512" ]]; then
                    # The SHA512 is broken here
                    continue;
                fi
                rlPhaseStartTest "$BACKEND: $METHOD: Sign & Verify (KEY $SIGN_KEY)"
                    rlLogInfo "generate data: -PSS methods should work on data > 512 bytes"
                    rlRun "head -c $DATA_SIZE </dev/urandom > data"
