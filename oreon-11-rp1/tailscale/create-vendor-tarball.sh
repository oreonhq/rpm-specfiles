...
git clone --branch v${version} --depth 1 https://github.com/tailscale/tailscale.git tailscale-${version}
pushd tailscale-${version}
...
# remove stuff we definitely don't use, shrink srpm
find cmd -type d -mindepth 1 -not -path "cmd/tailscale*" -exec rm -rfv {} +
rm -rf \
  k8s-operator \
  tstest

popd
XZ_OPT='-e -9 -T0' tar --exclude .git -cJf tailscale-${version}-vendored.tar.xz tailscale-${version}
#tar -I "zstd -T0 --ultra -22" --exclude .git -cf tailscale-${version}-vendored.tar.zst tailscale-${version}
rm -rf tailscale-${version}
