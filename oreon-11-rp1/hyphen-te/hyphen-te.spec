...
%global source0_file hyphen-te-0.7.0.tar.bz2
...
%(f=%{_sourcedir}/hyphen-te-0.7.0.tar.bz2; test -f "$f" || { echo "oreon: missing Source0 $f" >&2; exit 1; }; h=$(sha256sum "$f" | awk '{print $1}'); test "$h" = "68334d08ce32846509687c34c5650e83817050dd13371fdb43104402c58aa6b5" || { echo "oreon: Source0 SHA256 mismatch for hyphen-te-0.7.0.tar.bz2" >&2; exit 1; })
