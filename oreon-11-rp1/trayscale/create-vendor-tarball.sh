...
git clone --branch v${version} --depth 1 https://github.com/DeedleFake/trayscale.git trayscale-${version}
pushd trayscale-${version}
# go vendoring
go mod tidy
go mod vendor
...
popd
XZ_OPT='-9' tar --exclude .git -cJf trayscale-${version}-vendored.tar.xz trayscale-${version}
rm -rf trayscale-${version}
