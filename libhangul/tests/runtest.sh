...
rlJournalStart
    rlPhaseStartSetup
        rlShowPackageVersion libhangul
        rlRun -t -l "VERSION=$(rpm -q libhangul --queryformat='%{version}')" 0 "Get VERSION"
        FEDORA_VERSION=$(rlGetDistroRelease)
        rlLog "FEDORA_VERSION=${DISTRO_RELEASE}"
        rlRun "tmp=\$(mktemp -d)" 0 "Create tmp directory"
        rlRun "pushd $tmp"
        rlFetchSrcForInstalled "libhangul"
        ...
        if [ -d BUILD/libhangul-${VERSION}-build ]; then
            rlRun -t -l "pushd BUILD/libhangul-${VERSION}-build/libhangul-${VERSION}"
        else
            rlRun -t -l "pushd BUILD/libhangul-${VERSION}"
        fi
    rlPhaseEnd
